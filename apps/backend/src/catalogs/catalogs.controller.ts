...
export class CategoriesController {
  ...
  @RequirePermissions('CATEGORY_MANAGE') @Put(':id') async update(@Param('id') id: string, @Body() dto: UpdateCategoryDto) { return result(await this.service.updateCategory(id, dto), 'Categoría actualizada') }
  @RequirePermissions('CATEGORY_MANAGE') @Patch(':id/status') async status(@Param('id') id: string, @Body() dto: UpdateCatalogStatusDto) { return result(await this.service.setCategoryStatus(id, dto.status), 'Estado de categoría actualizado') }
  @RequirePermissions('CATEGORY_MANAGE') @Delete(':id') async remove(@Param('id') id: string) { return result(await this.service.deactivateCategory(id), 'Categoría desactivada') }
}
...
export class PrioritiesController {
  ...
  @RequirePermissions('PRIORITY_MANAGE') @Put(':id') async update(@Param('id') id: string, @Body() dto: UpdatePriorityDto) { return result(await this.service.updatePriority(id, dto), 'Prioridad actualizada') }
}
...
export class SlaPoliciesController {
  ...
  @RequirePermissions('SLA_MANAGE') @Put(':id') async update(@Param('id') id: string, @Body() dto: UpdateSlaPolicyDto) { return result(await this.service.updatePolicy(id, dto), 'Política SLA actualizada') }
}
...
export class CompaniesController {
  ...
  @Get(':id') async find(@Param('id') id: string) { return result(await this.service.findCompany(id)) }
}
