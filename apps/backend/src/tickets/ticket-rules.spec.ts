...
import { assertTicketSurvey, calculateSla, statusRequiresReason, TRANSITIONS } from './ticket-rules'
...
describe('Reglas de tickets', () => {
  ...
  it('impide saltar directamente de OPEN a RESOLVED', () => {
    expect(TRANSITIONS[TicketStatus.OPEN]).not.toContain(TicketStatus.RESOLVED)
  })
  ...
  it('calcula SLA vencido', () => {
    ...
    expect(sla.percentRemaining).toBe(0)
  })

  it('calcula SLA próximo a vencer', () => {
    const sla = calculateSla(
      new Date('2026-01-01T00:00:00Z'),
    ...
    expect(sla.level).toBe('orange')
  })

  it('conserva los roles documentados', () => {
    expect(Object.values(RoleCode)).toEqual(['ADMIN', 'SALES', 'SUPERVISOR', 'AGENT', 'CLIENT', 'REQUESTER'])
  })
  ...
  it('sólo el solicitante con permiso responde la encuesta de un ticket cerrado', () => {
    ...
    expect(() => assertTicketSurvey({ status: TicketStatus.OPEN, requester: { id: 'u1' } }, requester)).toThrow(
