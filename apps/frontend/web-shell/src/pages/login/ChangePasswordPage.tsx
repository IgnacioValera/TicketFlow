...
export function ChangePasswordPage() {
  ...
  const finish = async () => {
    await logout()
    navigate('/login', { replace: true, state: { notice: 'Tu contraseña se actualizó correctamente.' } })
  }
    ...
    <div className="min-h-screen bg-page">
      <header className="flex items-center justify-between border-b border-border bg-surface px-5 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <SecondaryButton onClick={() => void logout().then(() => navigate('/login', { replace: true }))}>
            Cerrar sesión
          </SecondaryButton>
        </div>
      </header>
      <main className="mx-auto max-w-lg px-5 py-10">
        <div className="rounded border border-border bg-surface p-6 sm:p-8">
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">Seguridad</p>
          <h1 className="mt-2 text-2xl font-semibold tracking-tight text-text">Cambiar contraseña</h1>
