...
type MockKnowledgeArticle = KnowledgeArticle & { status: 'ACTIVE' | 'INACTIVE' }
...
function buildMockArticles(): MockKnowledgeArticle[] {
  return SEED_ARTICLES.map((article, index) => ({
    id: `k${index + 1}`,
    title: article.title,
    ...
    category: article.categoryName
      ? { id: CATEGORY_IDS[article.categoryName] ?? '3', name: article.categoryName }
      : null,
...
type MockStore = typeof globalThis & {
  __ticketFlowMockKnowledge?: MockKnowledgeArticle[]
}

function getStore() {
  const store = globalThis as MockStore
  if (!store.__ticketFlowMockKnowledge) {
    store.__ticketFlowMockKnowledge = buildMockArticles()
  }
  return store.__ticketFlowMockKnowledge
}

export const mockKnowledgeArticles = getStore()

export function findMockKnowledgeArticle(id: string) {
  return getStore().find((article) => article.id === id && article.status === 'ACTIVE') ?? null
}

export function findMockKnowledgeArticleIncludingInactive(id: string) {
  return getStore().find((article) => article.id === id) ?? null
}
